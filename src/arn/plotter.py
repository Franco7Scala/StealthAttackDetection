...
def plot_ARN_loss(d_losses, g_losses, bce_losses, rec_losses, kldes, real_scores, fake_scores):
    ...
    plt.figure(figsize=(15, 10))
    plt.xlim(0, num_epochs + 1)
    plt.plot(range(1, num_epochs + 1), bce_losses[:num_epochs], 'yellow',
             label=r'$ -\mathbb{E}_{\tilde{x}} \left[\log p_{\theta} (y|\tilde{x}) \right]$')
    plt.plot(range(1, num_epochs + 1), rec_losses[:num_epochs], 'green',
             label=r'$\mathbb{E}_{\tilde{x}}\left[\log p(x|\tilde{x})\right]$')
